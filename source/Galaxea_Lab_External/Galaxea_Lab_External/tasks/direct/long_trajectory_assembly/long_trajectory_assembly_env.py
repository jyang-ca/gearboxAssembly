...
class LongTrajectoryAssemblyEnv(DirectRLEnv):
    ...
    def _setup_scene(self):
        ...
        # Optional cameras (disabled by default for RL training, enabled for evaluation)
        if self.cfg.enable_cameras:
            # Overhead camera (fixed world position, best for full workspace view)
            if hasattr(self.cfg, "overhead_camera_cfg") and self.cfg.overhead_camera_cfg is not None:
                self.overhead_camera = Camera(self.cfg.overhead_camera_cfg)
                self.scene.sensors["overhead_camera"] = self.overhead_camera
            
            if self.cfg.head_camera_cfg is not None:
                self.head_camera = Camera(self.cfg.head_camera_cfg)
                self.scene.sensors["head_camera"] = self.head_camera
            
            if self.cfg.left_hand_camera_cfg is not None:
                self.left_hand_camera = Camera(self.cfg.left_hand_camera_cfg)
                self.scene.sensors["left_hand_camera"] = self.left_hand_camera
                
            if self.cfg.right_hand_camera_cfg is not None:
