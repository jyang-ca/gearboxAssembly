...
class GalaxeaLabExternalEnv(DirectRLEnv):
    ...
    def _setup_scene(self):
        ...
        # Spawn cameras in template environment (env_0)
        # This ensures they exist before cloning, while allowing the Camera view to use the regex path for all envs.
        
        # Head Camera
        spawn_cfg_head = self.cfg.head_camera_cfg.copy()
        spawn_cfg_head.prim_path = spawn_cfg_head.prim_path.replace("env_.*", "env_0")
        self._camera_spawner_head = Camera(spawn_cfg_head)  # Retain reference to prevent GC
        
        view_cfg_head = self.cfg.head_camera_cfg.copy()
        view_cfg_head.spawn = None # Start view without spawning
        view_cfg_head.prim_path = view_cfg_head.prim_path.replace("env_.*", "env_0") # Explicit path
        self.head_camera = Camera(view_cfg_head)

        # Left Hand Camera
        spawn_cfg_left = self.cfg.left_hand_camera_cfg.copy()
        spawn_cfg_left.prim_path = spawn_cfg_left.prim_path.replace("env_.*", "env_0")
        self._camera_spawner_left = Camera(spawn_cfg_left)  # Retain reference to prevent GC

        view_cfg_left = self.cfg.left_hand_camera_cfg.copy()
        view_cfg_left.spawn = None
        view_cfg_left.prim_path = view_cfg_left.prim_path.replace("env_.*", "env_0")
        self.left_hand_camera = Camera(view_cfg_left)

        # Right Hand Camera
        spawn_cfg_right = self.cfg.right_hand_camera_cfg.copy()
        spawn_cfg_right.prim_path = spawn_cfg_right.prim_path.replace("env_.*", "env_0")
        self._camera_spawner_right = Camera(spawn_cfg_right)  # Retain reference to prevent GC

        view_cfg_right = self.cfg.right_hand_camera_cfg.copy()
        view_cfg_right.spawn = None
        view_cfg_right.prim_path = view_cfg_right.prim_path.replace("env_.*", "env_0")
        self.right_hand_camera = Camera(view_cfg_right)

        # Front Camera (Global)
        spawn_cfg_front = self.cfg.front_camera_cfg.copy()
        spawn_cfg_front.prim_path = spawn_cfg_front.prim_path.replace("env_.*", "env_0")
        self._camera_spawner_front = Camera(spawn_cfg_front)  # Retain reference to prevent GC

        view_cfg_front = self.cfg.front_camera_cfg.copy()
        view_cfg_front.spawn = None
        view_cfg_front.prim_path = view_cfg_front.prim_path.replace("env_.*", "env_0")
        self.front_camera = Camera(view_cfg_front)
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        self.rule_policy = GalaxeaRulePolicy(
            sim_utils.SimulationContext.instance(), 
            self.scene, 
            self.obj_dict,
            vision_estimator=getattr(self, 'vision_estimator', None)
        )
        ...
        self.save_hdf5_file_name = os.path.join(data_dir, 'data_' + datetime.now().strftime("%Y%m%d_%H%M%S") + '.hdf5')
    ...
    def step(self, action: torch.Tensor):
        ...
        print(f"Generate action at {current_time_s.item()} seconds")
        self.rule_policy.set_score(self.score)
        self.env_step_action, self.env_step_joint_ids = self.rule_policy.get_action()
        ...
        # Camera Parameters for Vision
        camera_params = {}
        for cam_name, cam in [('head', self.head_camera), ('left_hand', self.left_hand_camera), ('right_hand', self.right_hand_camera)]:
             # Ensure data is on CPU for estimator (or handle in estimator)
             if cam.data.quat_w_world.shape[0] > 0:
                 camera_params[cam_name] = {
                     'intrinsic': cam.data.intrinsic_matrices[0],
                     'pos_w': cam.data.pos_w[0],
                     'quat_w': cam.data.quat_w_world[0]
                 }
        
        # Prepare Obs Dict for Vision
        # self.obs_buf comes from _get_observations() which calls record_data logic
        # But we need direct depth for unscaling if normalized, or just use raw from cam if available.
        # Here we use the raw camera output which was just updated
        vision_obs = {}
        if self.head_camera.data.output["distance_to_image_plane"].shape[0] > 0:
             vision_obs['head'] = {'depth': self.head_camera.data.output["distance_to_image_plane"][0]}
        if self.left_hand_camera.data.output["distance_to_image_plane"].shape[0] > 0:
             vision_obs['left_hand'] = {'depth': self.left_hand_camera.data.output["distance_to_image_plane"][0]}
        if self.right_hand_camera.data.output["distance_to_image_plane"].shape[0] > 0:
             vision_obs['right_hand'] = {'depth': self.right_hand_camera.data.output["distance_to_image_plane"][0]}
             
        # Update Vision State (Moved to agent script level)
        # if camera_params: # Only update if we have camera data
        #     self.estimated_state = self.vision_estimator.update(vision_obs, camera_params)
            
        # Print for debug (optional)
        # if self.common_step_counter % 100 == 0:
        #     print(f"[Vision] Estimated State: {self.estimated_state}")
