...
class GalaxeaRulePolicy:
    ...
    obj_dict: dict, vision_estimator=None):
        self.sim = sim
        self.scene = scene
        ...
        self.recovery_step_count = None # Tensor of steps
        self.target_gear_id_for_recovery = None
        self.count_to_reset = 0
        self.count_to_reset = 0
        self.last_score_check_val = 0
        self.gear_checks = {1: False, 2: False, 3: False, 4: False}
        
        # Recovery timing: Pick(1s) -> Grasp(1s) -> Lift(1s) -> LiftHigh(1s) -> MoveBack(2s) -> Place(1s) -> Release(1s) -> Reset(1s)
        self.recovery_time_steps = torch.tensor([1.0, 1.0, 1.0, 1.0, 2.0, 1.0, 1.0, 1.0], device=sim.device)
        self.recovery_time_steps = torch.cumsum(self.recovery_time_steps, dim=0)
        self.recovery_total_steps = (self.recovery_time_steps / self.sim_dt).int()
    ...
    def get_object_pose(self, obj_name):
        ...
        if self.use_vision:
            ...
            if obj_name in poses and poses[obj_name]['available']:
                ...
                # --- Vision Height Correction ---
                # If Z is lower than table, clamp it to table + part height
                if pose['position'][2] < self.table_height:
                    part_h = 0.02 # Default (Standard gear)
                    if "ring" in obj_name:
                        part_h = 0.03
                    elif "reducer" in obj_name:
                        part_h = 0.05
                    elif "carrier" in obj_name:
                        part_h = 0.02
    ...
    def verify_vision_vs_gt(self):
        """
        Compare Vision Poses vs Ground Truth Poses and log errors.
        """
        if not self.use_vision: 
            return

        poses_vis = self.vision_estimator.get_3d_poses()
        
        parts = []
        for i in range(1, 5):
            parts.append(f"sun_planetary_gear_{i}")
        parts.append("ring_gear")
        parts.append("planetary_reducer")
        parts.append("planetary_carrier")
        
        print(f"\n[Validation] Vision vs GT (Count: {self.count})")
        print("-" * 60)
        
        for name in parts:
            if name not in self.obj_dict: continue
            
            # GT
            state_gt = self.obj_dict[name].data.root_state_w[0]
            pos_gt = state_gt[:3].cpu()
            
            # Vision
            if name in poses_vis and poses_vis[name]['available']:
                pos_vis = poses_vis[name]['position'].cpu()
                
                err = torch.norm(pos_vis - pos_gt).item()
                
                status = "OK"
                if err > 0.02: status = "WARN"
                
                print(f"[{status}] {name:<22} | Err: {err*1000:.1f}mm")
                print(f"      Vis: [{pos_vis[0]:.3f}, {pos_vis[1]:.3f}, {pos_vis[2]:.3f}]")
                print(f"      GT : [{pos_gt[0]:.3f}, {pos_gt[1]:.3f}, {pos_gt[2]:.3f}]")
            else:
                print(f"[FAIL] {name:<22} | Vision Not Available")
        print("-" * 60 + "\n")
    ...
    def pick_up_target_gear(self,
        ...
        if self.count == count_step[0]:
            ...
            if gear_id == 5:
                ...
                pose = self.get_object_pose('ring_gear')
                ring_gear_pos = pose['position'].unsqueeze(0)
                ring_gear_quat = pose['orientation'].unsqueeze(0)
                ...
                # Apply offsets here for latching
                ...
                # Update to use real-time position
                # Use vision-based pose estimation
                pose = self.get_object_pose('planetary_reducer')
        ...
        target_orientation = self.current_target_orientation.clone()
        ...
        if self.count >= count_step[0] and self.count < count_step[1]:
            ...
            arm_action, arm_joint_ids = self.move_robot_to_position(arm_entity_cfg, gripper_entity_cfg, diff_ik_controller, 
                                    target_position_h, target_orientation, None)
            
            # Combined Action: Move Arm + Open Gripper
            gripper_joint_ids = gripper_entity_cfg.joint_ids
            num_gripper_joints = len(gripper_joint_ids)
            gripper_action = torch.full((self.scene.num_envs, num_gripper_joints), 0.04, device=self.sim.device)
        ...
        if self.count >= count_step[1] and self.count < count_step[2]:
            ...
            # Combined Action: Move Arm + Open Gripper (Enforce Open)
            gripper_joint_ids = gripper_entity_cfg.joint_ids
            num_gripper_joints = len(gripper_joint_ids)
            gripper_action = torch.full((self.scene.num_envs, num_gripper_joints), 0.04, device=self.sim.device)
    ...
    def mount_gear_to_target(self,
        ...
        if gear_id == 4:
            ...
            # Use vision-based pose estimation
            pose = self.get_object_pose('sun_planetary_gear_4')
            root_state = torch.cat([pose['position'], pose['orientation']]).unsqueeze(0)
            if self.count == count_step[0]:
                self.current_target_position = root_state[:, :3].clone()
    ...
    def perform_recovery(self, gear_id, arm_entity_cfg, gripper_entity_cfg, diff_ik_controller):
        # Calculate relative count for recovery phase
        # Add 20 steps of latency for Vision Stabilization
        latency_steps = 20
        rel_count = self.count - self.recovery_start_count - latency_steps

        if rel_count < 0:
            # stabilized wait
            return None, None
        steps = self.recovery_total_steps
        ...
        if rel_count < steps[0]:
             ...
             # Must add TCP_offset_z because we are overwriting the Z component
             target_pos[:, 2] = gear_pos[:, 2] + self.grasping_height + 0.20 + self.TCP_offset_z # High Hover
             
             # Use fixed downward orientation (ignore gear tilt)
             target_ori = torch.tensor([[0.0, -1.0, 0.0, 0.0]], device=self.sim.device)
        ...
        if rel_count < steps[1]:
             ...
             gear_rot = pose['orientation'].unsqueeze(0)
             target_pos = gear_pos + torch.tensor([self.TCP_offset_x, 0.0, self.TCP_offset_z], device=self.sim.device)
             # Use actual gear Z for grasping
             # Must add TCP_offset_z because we are overwriting the Z component
             target_pos[:, 2] = gear_pos[:, 2] + self.grasping_height + self.TCP_offset_z
        ...
        if rel_count < steps[2]:
            gripper_joint_ids = gripper_entity_cfg.joint_ids
            action = torch.tensor([[0.0]], device=self.sim.device) # Close
            return action, gripper_joint_ids
            
        # 4. Lift High (Vertical only)
        if rel_count < steps[3]:
             ...
             # Lift vertically to a safe height (Stable Reference)
             # Use table_height to avoid feedback loop with moving gear
             target_pos[:, 2] = self.table_height + self.grasping_height + 0.20 + self.TCP_offset_z
             ...
             # Append Gripper Close Action to ensure it holds the gear
             gripper_action = torch.full((len(gripper_entity_cfg.joint_ids),), 0.0, device=self.sim.device).unsqueeze(0)
             action = torch.cat([action, gripper_action], dim=1)
             joint_ids = joint_ids + gripper_entity_cfg.joint_ids
             
             return action, joint_ids

        # 5. Move to Initial (High)
        if rel_count < steps[4]:
             # Initial Pos
             init_pos = self.initial_root_state[f"sun_planetary_gear_{gear_id}"][:, :3].clone()
             target_pos = init_pos + torch.tensor([self.TCP_offset_x, 0.0, self.TCP_offset_z], device=self.sim.device)
             target_pos[:, 2] = self.table_height + self.grasping_height + 0.20 + self.TCP_offset_z # Keep High
        ...
        if rel_count < steps[5]:
             init_pos = self.initial_root_state[f"sun_planetary_gear_{gear_id}"][:, :3].clone()
             target_pos = init_pos + torch.tensor([self.TCP_offset_x, 0.0, self.TCP_offset_z], device=self.sim.device)
             target_pos[:, 2] = self.table_height + self.grasping_height + self.TCP_offset_z
        ...
        if rel_count < steps[6]:
            gripper_joint_ids = gripper_entity_cfg.joint_ids
            num_gripper_joints = len(gripper_joint_ids)
            action = torch.full((num_gripper_joints,), 0.04, device=self.device).unsqueeze(0)
            return action, gripper_joint_ids
        ...
        if rel_count < steps[7]:
             init_pos = self.initial_root_state[f"sun_planetary_gear_{gear_id}"][:, :3].clone()
             target_pos = init_pos + torch.tensor([self.TCP_offset_x, 0.0, self.TCP_offset_z], device=self.sim.device)
             target_pos[:, 2] = self.table_height + self.grasping_height + 0.15 + self.TCP_offset_z
    ...
    def get_action(self):
        ...
        # --- PERIODIC VISION VALIDATION ---
        if self.count % 50 == 0:
            self.verify_vision_vs_gt()
            
        if self.recovery_mode:
            gear_id = self.target_gear_id_for_recovery
            
            # Gear 4 might not be in map because it doesn't go to a pin (center gear)
            # Default to right arm as per main logic
            if gear_id == 4:
                current_arm_str = 'right'
            else:
            ...
            action, joint_ids = self.perform_recovery(gear_id, current_arm, current_gripper, self.diff_ik_controller)
            
            # Check if recovery finished
            rel_count = self.count - self.recovery_start_count
            if rel_count >= self.recovery_total_steps[-1]:
                print(f"[INFO] Recovery finished. Rewinding to count {self.count_to_reset}")
                self.recovery_mode = False
                self.count = self.count_to_reset
                
                # Reset flags based on where we are rewinding to
                if self.target_gear_id_for_recovery <= 1: self.gear_checks[1] = False
                if self.target_gear_id_for_recovery <= 2: self.gear_checks[2] = False
                if self.target_gear_id_for_recovery <= 3: self.gear_checks[3] = False
                if self.target_gear_id_for_recovery <= 4: self.gear_checks[4] = False
            
            return action, joint_ids # Return early logic override
        

        # --- Failure Checks (Robust) ---
        # Gear 1 Check (At start of Step 3)
        if not self.gear_checks[1] and self.count >= self.count_step_3[0]:
            self.gear_checks[1] = True
            if self.current_score < 1:
                print("[WARN] Gear 1 Mount Failed! Triggering Recovery.")
                self.recovery_mode = True
                self.recovery_start_count = self.count
                self.target_gear_id_for_recovery = 1
                # Rewind to Pick Up Phase (Step 1) to retry the entire sequence
                self.count_to_reset = self.count_step_1[0]
                return action, joint_ids

        # Gear 2 Check (At start of Step 5)
        if not self.gear_checks[2] and self.count >= self.count_step_5[0]:
            self.gear_checks[2] = True
            if self.current_score < 2:
                print("[WARN] Gear 2 Mount Failed! Triggering Recovery.")
                self.recovery_mode = True
                self.recovery_start_count = self.count
                self.target_gear_id_for_recovery = 2
                self.count_to_reset = self.count_step_3[0]
                return action, joint_ids

        # Gear 3 Check (At start of Step 8)
        if not self.gear_checks[3] and self.count >= self.count_step_8[0]:
            self.gear_checks[3] = True
            if self.current_score < 3:
                print("[WARN] Gear 3 Mount Failed! Triggering Recovery.")
                self.recovery_mode = True
                self.recovery_start_count = self.count
                self.target_gear_id_for_recovery = 3
                self.count_to_reset = self.count_step_6[0]
                return action, joint_ids

        # Gear 4 Check (At start of Step 10)
        if not self.gear_checks[4] and self.count >= self.count_step_10[0]:
            self.gear_checks[4] = True
            if self.current_score < 4:
                print("[WARN] Gear 4 Mount Failed! Triggering Recovery.")
                self.recovery_mode = True
                self.recovery_start_count = self.count
                self.target_gear_id_for_recovery = 4
                self.count_to_reset = self.count_step_8[0]
                return action, joint_ids
